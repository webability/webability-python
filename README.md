...
Cliente oficial en Python para conectarse a los servicios de [WebAbility](https://www.webability.info) — la plataforma que ofrece DNS gestionado, procesamiento de imágenes/CDN, envío de correo transaccional y (próximamente) transcodificación de video y email marketing masivo, todos expuestos a través de una única API HTTP en `https://api.webability.info`.

🚧 **En construcción.** Es el equivalente en Python de [webability-go](https://github.com/webability/webability-go) (implementación de referencia) — mismo esquema de autenticación (ClientID + Token, firma HMAC-SHA256, headers `X-WA-Client`/`X-WA-Timestamp`/`X-WA-Digest`) y mismos endpoints (`dns`, `image`, `mail`, y próximamente `video`/`marketing`).

## Instalación (cuando esté publicado)

```bash
pip install webability-python
```
